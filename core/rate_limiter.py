...
429 response shape (via error_boundary envelope):
  {
    "ok": false,
    "error": {
      "code":           "RATE_LIMITED",
...
Usage:
    # In server.py, after app = FastAPI(...):
    from core.rate_limiter import RateLimitMiddleware
    app.add_middleware(RateLimitMiddleware)  # global IP limiter

    # On individual expensive endpoints:
    from core.rate_limiter import rate_limit
    @app.post("/gaians/{slug}/chat")
    async def chat(slug, req, user=Depends(require_auth),
                   _rl=Depends(rate_limit(max_requests=30, window_seconds=60))):
        ...
...
def _sliding_window_check(
    ...
    if count >= max_requests:
        # Time until the oldest request slides out
...
def rate_limit(
    ...
    scope:          str = "user",   # key prefix for namespacing
) -> Callable:
    """
    FastAPI dependency factory. Returns a dependency that enforces a
    per-authenticated-user sliding window limit for the decorated endpoint.

    Args:
        max_requests:   Maximum requests in the window. Default: 30.
        window_seconds: Window length in seconds. Default: 60.
        scope:          Key prefix (e.g. "chat", "query") for isolation.
                        Two endpoints with different scopes have independent
    ...
    async def _dependency(request: Request) -> None:
        # Try to extract user_id from JWT for per-user keying;
        # fall back to IP if no auth header present.
        from core.auth import optional_auth, HTTPBearer
